"""
618 + FLIP ZONE VALIDATOR (HUNTER MODE)

HUNTER MODE ALERT TIMING:
- Alert when price breaks BELOW 0.382
- AND flip zone aligns with 0.618
- AND price is moving toward the 0.618 + flip zone
- This is an EARLY approach alert
"""
...
try:
    from setup_validators.hunter_mode import check_fib_break_with_mapping, get_current_fib_alignment
except ImportError:
...
def validate_618(candles: List[Dict], symbol: str, structure: Dict = None) -> ValidationResult:
    """Validate 618 + Flip Zone with HUNTER MODE."""
    ...
    passes_618_gate = structure.get('passes_618fz_gate', False)
    has_valid_breakout = structure.get('ath_breakout', False) or structure.get('major_high_break', False)
    
    if not passes_618_gate and not has_valid_breakout:
        result.reject_reason = "No valid breakout structure"
        logger.info(f"   [618-VAL] {symbol}: ❌ REJECTED - No valid breakout structure")
        return result
    ...
    # HUNTER MODE: Check fib break with mapping
    fib_break_data = {'fib_broken': False, 'flip_zone_aligned': False, 'ready_to_alert': False}
    if check_fib_break_with_mapping:
        fib_break_data = check_fib_break_with_mapping(structure, 0.382, 0.618)
    else:
        # Fallback check
        fib_break_data['fib_broken'] = current_price < fib_382
        for fz in flip_zones:
            fz_level = fz.get('level', 0) if isinstance(fz, dict) else 0
            if fz_level > 0 and abs(fz_level - fib_618) <= range_size * 0.05:
                fib_break_data['flip_zone_aligned'] = True
                break
        fib_break_data['approaching_target'] = fib_break_data['fib_broken'] and current_price > fib_618
    ...
    result.layers.append(ValidationLayer(
        ...
        reason=f"Broke 382: {'✓' if fib_broken else '✗'} | FZ at 618: {'✓' if flip_zone_aligned else '✗'} | Approaching: {'✓' if approaching else '✗'}",
        weight=1.5
    ))
    
    if hunter_ready:
        logger.info(f"   [618-VAL] hunter_approach: ✓ - Broke 382, approaching 618 with valid FZ ✓")
    
    # LAYER: Expansion
    expansion_passed = impulse_pct >= 60
    expansion_score = 25 if impulse_pct >= 100 else 20 if impulse_pct >= 80 else 15 if impulse_pct >= 60 else 0
    result.layers.append(ValidationLayer(
    ...
    # LAYER: Flip Zone at 618
    result.layers.append(ValidationLayer(
        layer_name="flip_zone",
        passed=flip_zone_aligned,
        score=20 if flip_zone_aligned else 0,
        reason=f"FZ aligned with 618: {'✓' if flip_zone_aligned else '✗'}",
        weight=1.3
    ))
    
    # LAYER: Retracement (Hunter Mode accepts early)
    ...
    elif 30 <= retracement_pct < 50:
        depth_passed = True
        depth_score = 18
        depth_reason = f"Approaching 618 ({retracement_pct:.0f}%)"
    elif 50 <= retracement_pct <= 68:
        depth_passed = True
        depth_score = 20
        depth_reason = f"At 618 zone ({retracement_pct:.0f}%)"
    else:
        depth_passed = False
        depth_score = 0
        depth_reason = f"Below 618 ({retracement_pct:.0f}%)"
    ...
    result.final_score = min(100, int((total_score / 110) * 100))
    
    # Pass criteria: Hunter ready OR at zone with flip zone
    hunter_valid = hunter_ready and expansion_passed
    zone_valid = depth_passed and flip_zone_aligned and expansion_passed
    result.passed = (hunter_valid or zone_valid) and result.final_score >= 60
    ...
    if result.passed:
        if hunter_ready and retracement_pct < 45:
            result.stage = 1
            result.stage_label = "BROKE 382 - APPROACHING 618"
        elif retracement_pct < 58:
            result.stage = 2
            result.stage_label = "APPROACHING 618"
        else:
            result.stage = 3
            result.stage_label = "AT 618 ZONE"
    ...
    logger.info(f"   [618-VAL] {symbol}: {status} {mode} Score={result.final_score} Grade={result.final_grade}")
