import os
from monitoring import daily_command, debug_command, missed_command, monitor_command
import re
import json
import asyncio
import logging
...
DATA_DIR = Path("/data")
SCANNER_PAUSE_FILE = DATA_DIR / "scanner_paused.flag"
...
# ══════════════════════════════════════════════════════════════════════════════
# SCANNER PAUSE/RESUME (file-based for speed)
# ══════════════════════════════════════════════════════════════════════════════

async def pause_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Pause the scanner - Owner only"""
    if not is_owner(update.effective_user.id):
        return
    SCANNER_PAUSE_FILE.touch()
    await update.message.reply_text("⏸️ Scanner PAUSED\n\n<i>Use /resume to start again</i>", parse_mode='HTML')

async def resume_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Resume the scanner - Owner only"""
    if not is_owner(update.effective_user.id):
        return
    if SCANNER_PAUSE_FILE.exists():
        SCANNER_PAUSE_FILE.unlink()
    await update.message.reply_text("▶️ Scanner RESUMED", parse_mode='HTML')
...
def main():
    ...
    application.add_handler(CommandHandler("training_restore", training_restore_command))
        ...
        # Scanner control commands (Owner only)
